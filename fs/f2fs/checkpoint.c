...
static void __checkpoint_and_complete_reqs(struct f2fs_sb_info *sbi)
{
	...
	ret = __write_checkpoint_sync(sbi, NULL, true);
...
static int flush_remained_checkpoint_cmd(struct f2fs_sb_info *sbi,
			struct checkpoint_cmd *nowait_cmd)
{
	struct f2fs_ckpt_cmd_control *ccc = sbi->ccc_info;
...
int f2fs_create_checkpoint_cmd_control(struct f2fs_sb_info *sbi)
{
	...
	ccc = f2fs_kzalloc(sbi, sizeof(struct f2fs_ckpt_cmd_control),
					GFP_KERNEL);
	if (!ccc)
		return -ENOMEM;
...
init_thread:
	ccc->ckpt_task = kthread_run(issue_checkpoint_cmd_thread, sbi,
				"f2fs_ckpt-%u:%u", MAJOR(dev), MINOR(dev));
