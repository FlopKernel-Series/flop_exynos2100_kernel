...
#ifdef CONFIG_EVASION_DISP_DET
		if (sec_get_feat(SEC_FEAT_EVASION_DISP_DET)) {
...
static int __panel_seq_res_init(struct panel_device *panel)
{
	...
	if (sec_get_feat(SEC_FEAT_SUPPORT_GM2_FLASH)) {
...
int panel_display_on(struct panel_device *panel)
{
	...
	if (sec_get_feat(SEC_FEAT_SUPPORT_TIG)) {
		if (panel->state.cur_state == PANEL_STATE_ALPM) {
...
#ifdef CONFIG_SUPPORT_POC_SPI
	if (sec_get_feat(SEC_FEAT_SUPPORT_POC_SPI)) {
