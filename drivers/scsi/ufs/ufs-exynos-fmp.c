...
void exynos_ufs_fmp_config(struct ufs_hba *hba, bool init)
{
	if (init) {
		/*
		 * The android12-5.4 UFS core no longer carries Samsung's
		 * old crypto_vops passthrough model. Keep the live Exynos
		 * integration that this tree still uses: enlarged PRDT
		 * entries plus the secure monitor side configuration.
		 */
